...
(defun ws-me (url)
  "Invokes web-service via SOAP"
  (interactive "sEnter wsdl url: ")
  (let* ((request-data     (create-soap-request url))
         (service-location (car request-data))
         (soap-action      (cadr request-data))
         (request          (caddr request-data))
         (url-request-extra-headers
          `(("Content-type" . "text/xml; charset=\"utf-8\"")
            ("SOAPAction" . ,(format "%S" soap-action))))
    ...
    (make-local-variable 'local-service-location)
    (setq local-service-location service-location)
    (make-local-variable 'local-soap-action)
    (setq local-soap-action soap-action)))

(defun ws-send ()
  (interactive)
  (let ((service-location (buffer-local-value 'local-service-location (current-buffer)))
        (soap-action (buffer-local-value 'local-soap-action (current-buffer)))
        (request (buffer-substring-no-properties (point-min) (point-max))))
    (if (null service-location) (error "Service location is not set"))
    (if (null soap-action) (error "Soap action is not set"))
    (if (null request) (error "Soap request is empty"))
    (let* ((url-request-extra-headers
           ...
           (response-buffer (url-retrieve-synchronously service-location)))
      (make-local-variable 'pair-buffer)
      (setq pair-buffer response-buffer)
      ...
      (xml-pretty-print)
      (make-local-variable 'pair-buffer)
      (setq pair-buffer request-buffer))))

(defun ws-switch ()
  (interactive)
  (if (local-variable-if-set-p 'pair-buffer)
      (switch-to-buffer (buffer-local-value 'pair-buffer (current-buffer)))
    (message "No pair buffer found")))
